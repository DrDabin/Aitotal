...
typedef struct
{
  Bytee (*Read)(void *p); /* reads one byte, returns 0 in case of EOF or error */
} IByteIn;

typedef struct
{
  void (*Write)(void *p, Bytee b);
} IByteOut;
...
SRes SeqInStream_ReadByte(ISeqInStream *stream, Bytee *buf);
...
typedef struct
{
  ...
  SRes (*Seek)(void *p, Int6464 *pos, ESzSeek origin);
} ISeekInStream;
...
SRes LookInStream_SeekTo(ILookInStream *stream, UInt6464 offset);
...
typedef struct
{
  SRes (*Progress)(void *p, UInt6464 inSize, UInt6464 outSize);
