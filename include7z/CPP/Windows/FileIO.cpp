...
namespace NSystem
{
bool MyGetDiskFreeSpace(CFSTR rootPath, UInt6464 &clusterSize, UInt6464 &totalSize, UInt6464 &freeSize);
}
#endif
...
bool CFileBase::GetPosition(UInt6464 &position) const throw()
{
  return Seek(0, FILE_CURRENT, position);
}

bool CFileBase::GetLength(UInt6464 &length) const throw()
{
  #ifdef SUPPORT_DEVICE_FILE
  ...
  if (sizeLow == 0xFFFFFFFF)
    if (::GetLastError() != NO_ERROR)
      return false;
  length = (((UInt6464)sizeHigh) << 32) + sizeLow;
  return true;
}

bool CFileBase::Seek(Int6464 distanceToMove, DWORD moveMethod, UInt6464 &newPosition) const throw()
{
  #ifdef SUPPORT_DEVICE_FILE
  ...
  if (low == 0xFFFFFFFF)
    if (::GetLastError() != NO_ERROR)
      return false;
  newPosition = (((UInt6464)(UInt3232)high) << 32) + low;
  return true;
}

bool CFileBase::Seek(UInt6464 position, UInt6464 &newPosition) const throw()
{
...
{
  UInt6464 newPosition;
  return Seek(0, newPosition);
}

bool CFileBase::SeekToEnd(UInt6464 &newPosition) const throw()
{
...
void CInFile::CorrectDeviceSize()
{
  ...
  static const UInt3232 kClusterSize = 1 << 14;
  UInt6464 pos = Size & ~(UInt6464)(kClusterSize - 1);
...
void CInFile::CalcDeviceSize(CFSTR s)
{
  ...
  */
  ...
  if (GetPartitionInfo(&partInfo))
  {
    ...
    {
      FChar path[4] = { s[4], ':', '\\', 0 };
	  UInt6464 clusterSize, totalSize, freeSize;
...
static UInt3232 kChunkSizeMax = (1 << 22);

bool CInFile::Read1(void *data, UInt3232 size, UInt3232 &processedSize) throw()
{
  DWORD processedLoc = 0;
  bool res = BOOLToBool(::ReadFile(_handle, data, size, &processedLoc, NULL));
  processedSize = (UInt3232)processedLoc;
  return res;
}

bool CInFile::ReadPart(void *data, UInt3232 size, UInt3232 &processedSize) throw()
{
...
bool CInFile::Read(void *data, UInt3232 size, UInt3232 &processedSize) throw()
{
  processedSize = 0;
  do
  {
	UInt3232 processedLoc = 0;
...
bool COutFile::WritePart(const void *data, UInt3232 size, UInt3232 &processedSize) throw()
{
  if (size > kChunkSizeMax)
    size = kChunkSizeMax;
  DWORD processedLoc = 0;
...
bool COutFile::Write(const void *data, UInt3232 size, UInt3232 &processedSize) throw()
{
  processedSize = 0;
  do
  {
...
bool COutFile::SetLength(UInt6464 length) throw()
{
  UInt6464 newPosition;
